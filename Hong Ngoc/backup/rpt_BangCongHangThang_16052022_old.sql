...
@condition VARCHAR(MAX) =" and (at.MonthYear = '2022-05-01') and (at.ProfileID in ('6F6E121B-299E-4BD6-95FC-6E8FBC2C8FC3')) ",  
@PageIndex INT = 1,  
@PageSize INT = 1000,  
...
begin
		set @Top = ' top 0 ';
		SET @condition =  ' and (at.MonthYear = ''2022-05-01'') and (at.ProfileID in (''6F6E121B-299E-4BD6-95FC-6E8FBC2C8FC3'')) '
END;
...
from		
			...
			SELECT		hp.id,hp.CodeEmp,hp.ProfileName,cp.PositionName,co.OrgStructureName  
						...
						,SL1,SL2,KoDK_CoAn.DienGiai1,DienGiai2  
			from		#Att_AttendanceTable at  
			...
			left join	Cat_LeaveDayType cl1 on cl1.id=at.LeaveDay1Type
			left join	Cat_LeaveDayType cl2 on cl2.id=at.LeaveDay2Type
			left join	Cat_LeaveDayType cl3 on cl3.id=at.LeaveDay3Type
			left join	Cat_LeaveDayType cl4 on cl4.id=at.LeaveDay4Type
			left join	Cat_LeaveDayType cl5 on cl5.id=at.LeaveDay5Type
			left join	Cat_LeaveDayType cl6 on cl6.id=at.LeaveDay6Type
			--- Lay 1 lan moi nhat
			left join	(select		ProfileID,cj.Code,ap.Note,ap.WorkDate,ap.ActualHours as ActualHours1 
						from		Att_ProfileTimeSheet ap
						left join	Cat_JobType cj on cj.id=ap.JobTypeID  
						where		JobTypeID in (select id from Cat_JobType where Code in (''CongThieu'')) and ap.IsDelete is null
						) CC on CC.profileid=at.profileid and CC.WorkDate>=at.DateStart and CC.WorkDate<=at.DateEnd
  
			left join (	select		ProfileID,cj.Code,ap.Note,ap.WorkDate,ap.ActualHours 
						from		Att_ProfileTimeSheet ap  
						left join	Cat_JobType cj on cj.id=ap.JobTypeID  
						where		JobTypeID in (select id from Cat_JobType where Code in (''CongThua'')) and ap.IsDelete is null
						) CC1 on CC1.profileid=at.profileid and CC1.WorkDate>=at.DateStart and CC1.WorkDate<=at.DateEnd  
			 '
			SET @query2 = ' 
			...
			left join (
						select		ProfileID,AttendanceTableID,sum(TienPhatMuon) as Tien_Muon, sum(SoLanMuon) as SoLanMuon 
						from   
									(select		case  
												...
												,case when aati.LateEarlyMinutes >3 then 1 else 0 end as SoLanMuon  
						...
						) TienMuon on TienMuon.AttendanceTableID=at.id and TienMuon.ProfileID=at.ProfileID  
   
			left join (	
						select AttendanceTableID,count(id) as SL_ThieuInOut from Att_AttendanceTableItem   
			...
			left join (	
						...
						and AttendanceTableID in (select id from Att_AttendanceTable where IsDelete is null -- xoa di -- and monthyear=''2021-09-01'')  
			...
			--Canteen  
			left join (	
						select		cms.ProfileID,DATEADD(month, DATEDIFF(month, 0, TimeLog), 0) as Thang,count (cms.id) SL1, STRING_AGG(concat('' '',DATEPART(DAY,TimeLog),''/'',DATEPART(MONTH,TimeLog)),'','') DienGiai1
			...
			left join	(
						select		cm.ProfileID,DATEADD(month, DATEDIFF(month, 0, cm.DateMeal), 0) Thang,count(cm.id) SL2,STRING_AGG(concat('' '',DATEPART(DAY,DateMeal),''/'',DATEPART(MONTH,DateMeal)),'','') DienGiai2  
