...
@condition nvarchar(max) =  " and (MonthYear = '2022-02-01') and (CheckData like N'%E_POSTS%') ",
@PageIndex int = 1,
@PageSize int = 50000,
...
BEGIN
	...
	IF(@index > 0)
	BEGIN
	SET @EmpStatus = REPLACE(@EmpStatus,'''E_PROFILE_NEW''',' (hp.DateHire <= @DateEndAdvance AND hp.DateHire >= @DateStart ) ')
	END
	SET @index = charindex('E_PROFILE_ACTIVE',@EmpStatus,0) 
	IF(@index > 0)
	BEGIN
	SET @EmpStatus = REPLACE(@EmpStatus,'''E_PROFILE_ACTIVE''','(hp.DateQuit IS NULL OR hp.DateQuit > @DateEndAdvance)')
	END
	SET @index = charindex('E_PROFILE_QUIT',@EmpStatus,0)
	IF(@index > 0)
	BEGIN
	SET @EmpStatus = REPLACE(@EmpStatus,'''E_PROFILE_QUIT''',' (hp.DateQuit <= @DateEndAdvance AND hp.DateQuit >= @DateStart) ')	
	END
END
