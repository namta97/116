...
SET @Query = N'
	...
	select		hp.ID AS ProfileID , hp.CodeEmp, hp.ProfileName, cou.E_BRANCH AS DivisionName ,cou.E_UNIT AS CenterName,cou.E_DIVISION AS DepartmentName,cou.E_DEPARTMENT AS SectionName,cou.E_TEAM AS UnitName
				...
				--So ngay lam viec
				,CASE WHEN att.ID IS NOT NULL THEN dbo.fnc_NumberOfExceptWeekends(@DateStart,@DateEnd) ELSE 0 END AS StdWorkDayCount
	...
	inner join	#tblPermission tp
		on		tp.ID = hwh.ProfileID
	OUTER APPLY (
...
set @Query4 = N'
	...
	,ISNULL(RealWorkDayCount,0) + ISNULL(HLD,0)+ISNULL(A,0)+ISNULL(AN,0)+ISNULL(AN1,0)+ISNULL(ADV,0)+ISNULL(F,0) +ISNULL(M,0)+ISNULL(CM,0)+ISNULL(ML,0)+ISNULL(MLT,0)+ISNULL(C,0) 
...
set @Query5 = N'
	...
	,NULL AS "hp.CodeEmp",NULL AS "cos.OrderNumber",NULL AS "hwh.EmploymentType",NULL AS "hwh.SalaryClassID" ,NULL AS "hwh.PositionID", NULL AS "hwh.JobTitleID", NULL AS "hp.DateHire", NULL AS "hp.DateEndProbation"
	, NULL AS "hp.DateQuit",NULL AS "hwh.WorkPlaceID",NULL AS "hwh.EmployeeGroupID", NULL AS "hwh.LaborType"
	,NULL AS "CheckData"
	INTO #Results
...
set @queryPageSize = N'
	...
	ORDER BY STT
	drop table #DataMain, #LeaveDayPivot, #OvertimePivot, #BusinessTravelPivot, #OvertimePlan, #LateEarlyMinutes, #RealWorkDayCount, #LenChinhThuc,#Att_AttendanceTable, #NightShiftHours,#WorkHistory
	'
...
Completion time: 2022-01-08T11:19:54.2096928+07:00
