ALTER proc dr_BCCongChiTietThang
      @Condition varchar(max) = " and (MonthYear = ''2021/04/01'') "
...
-- Tach dieu kien
			...
			set @index = charindex('(NotHaveData ','('+@tempID,0) 
			if(@index > 0)
			BEGIN
				set @tempCodition = 'AND ('+@tempID
				set @condition = REPLACE(@condition,@tempCodition,'')
				set @NotHaveData = @tempCodition
			END

			set @index = charindex('(IsHaveData ','('+@tempID,0) 
			if(@index > 0)
			BEGIN
				set @tempCodition = 'AND ('+@tempID
				set @condition = REPLACE(@condition,@tempCodition,'')
				set @IstHaveData = @tempCodition
			END
...
select  Data.ID
	  ...
	  , hp.DateHire
	  , hp.DateQuit
      , row_number() over (order by cou.E_COMPANY, cou.E_BRANCH, cou.E_UNIT,cou.E_DIVISION, cou.E_DEPARTMENT, cou.E_TEAM, cou.E_SECTION, hp.ProfileName) as STT
into    #TbWorkHistory
from    (select ' + @Top + '
				hwh.ID
              , hwh.ProfileID
...
select  twh.*
      ...
      , tsbl.*
      , tsld.*
 	  , nullif(isnull(tsld.TotalPaidLeaveDay,0) + isnull(tsld.HLD,0) + isnull(tsbl.TotalBusinessTravelDays,0) + isnull(tswd.TotalWorkHours,0),0) as TotalPaidDays
      , tso.*
      , tswd.*
	  , nullif(tso.NS2 + tso.NS3 + tso.NS4 + isnull(tswd.NS1,0),0) as TotalNS
	  ...
	  , NULL AS "IsHaveData"
from    #TbWorkHistory as twh
		left join #TbDetail as td on td.ProfileID = twh.ProfileID
