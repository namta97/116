...
-- Dieu kien @MonthYearCondition
        ...
        select  @ListShiftCode = coalesce(@ListShiftCode + ',', '') + quotename(cs.CodeStatic)
        from    
			(SELECT DISTINCT ISNULL(CodeStatic,Code) AS CodeStatic
			FROM	Cat_Shift
			where   IsDelete is NULL
			) cs
		ORDER BY cs.CodeStatic
		;
...
WHERE		hwh.IsDelete is null
	...
	AND		( hp.DateQuit IS NULL OR hp.DateQuit > @DateEnd)
...
select ' + @Top
		...
		,cs.ShiftCode
		,CASE WHEN 1=1 THEN
		CASE WHEN cs.ShiftCode = ''2'' AND 1.0 -  ( ISNULL(LEAVEDETAIL.LeaveDays1,0) + ISNULL(LEAVEDETAIL.LeaveDays2,0) + ISNULL(LEAVEDETAIL.LeaveDays3,0) )  >= 0.5 AND LeaveDays3 IS NULL then 0.5 
		WHEN cs.ShiftCode = ''3'' AND 1.0 -  ( ISNULL(LEAVEDETAIL.LeaveDays1,0) + ISNULL(LEAVEDETAIL.LeaveDays2,0) + ISNULL(LEAVEDETAIL.LeaveDays3,0) )  = 1 then 7.5
		WHEN cs.ShiftCode = ''3'' AND 1.0 -  ( ISNULL(LEAVEDETAIL.LeaveDays1,0) + ISNULL(LEAVEDETAIL.LeaveDays2,0) + ISNULL(LEAVEDETAIL.LeaveDays3,0) )  = 0.5 AND LeaveDays2 IS NULL then 4.0
		WHEN cs.ShiftCode = ''3'' AND 1.0 -  ( ISNULL(LEAVEDETAIL.LeaveDays1,0) + ISNULL(LEAVEDETAIL.LeaveDays2,0) + ISNULL(LEAVEDETAIL.LeaveDays3,0) )  = 0.5 AND LeaveDays3 IS NULL then 3.5
		END END AS NS1
		,
			--CASE WHEN  cs.ShiftCode is not null THEN 
            CASE WHEN
				1.0 - 1.0 + ISNULL(BUSINESSDETAIL.BusinessDay1,0) + ISNULL(BUSINESSDETAIL.BusinessDay2,0) + ISNULL(BUSINESSDETAIL.BusinessDay3,0) - ( ISNULL(LEAVEDETAIL.LeaveDays1,0) + ISNULL(LEAVEDETAIL.LeaveDays2,0) + ISNULL(LEAVEDETAIL.LeaveDays3,0) ) >= 0
				THEN 1.0 - 1.0 + ISNULL(BUSINESSDETAIL.BusinessDay1,0) + ISNULL(BUSINESSDETAIL.BusinessDay2,0) + ISNULL(BUSINESSDETAIL.BusinessDay3,0) - ( ISNULL(LEAVEDETAIL.LeaveDays1,0) + ISNULL(LEAVEDETAIL.LeaveDays2,0) + ISNULL(LEAVEDETAIL.LeaveDays3,0) )
				+ CASE WHEN 1.0 - 1.0 + ISNULL(BUSINESSDETAIL.BusinessDay2,0) - ISNULL(LEAVEDETAIL.LeaveDays2,0) > 0 THEN 1.0 - 1.0 + ISNULL(BUSINESSDETAIL.BusinessDay2,0) - ISNULL(LEAVEDETAIL.LeaveDays2,0) ELSE 0 END
				+ CASE WHEN 1.0 - 1.0 + ISNULL(BUSINESSDETAIL.BusinessDay3,0) - ISNULL(LEAVEDETAIL.LeaveDays3,0) > 0 THEN 1.0 - 1.0 + ISNULL(BUSINESSDETAIL.BusinessDay3,0) - ISNULL(LEAVEDETAIL.LeaveDays3,0) ELSE 0 END
		...
		,CASE WHEN cs.ShiftCode is not null THEN 
		    CASE WHEN
				1.0 - 1.0 + ISNULL(BUSINESSDETAIL.BusinessDay1,0) + ISNULL(BUSINESSDETAIL.BusinessDay2,0) + ISNULL(BUSINESSDETAIL.BusinessDay3,0) - ( ISNULL(LEAVEDETAIL.LeaveDays1,0) + ISNULL(LEAVEDETAIL.LeaveDays2,0) + ISNULL(LEAVEDETAIL.LeaveDays3,0) ) >= 0
				THEN 
				1.0 - ( ISNULL(BUSINESSDETAIL.BusinessDay1,0) + ISNULL(BUSINESSDETAIL.BusinessDay2,0) + ISNULL(BUSINESSDETAIL.BusinessDay3,0)
				+ CASE WHEN 1.0 - 1.0 + ISNULL(BUSINESSDETAIL.BusinessDay2,0) - ISNULL(LEAVEDETAIL.LeaveDays2,0) > 0 THEN 1.0 - 1.0 + ISNULL(BUSINESSDETAIL.BusinessDay2,0) - ISNULL(LEAVEDETAIL.LeaveDays2,0) ELSE 0 END
				+ CASE WHEN 1.0 - 1.0 + ISNULL(BUSINESSDETAIL.BusinessDay3,0) - ISNULL(LEAVEDETAIL.LeaveDays3,0) > 0 THEN 1.0 - 1.0 + ISNULL(BUSINESSDETAIL.BusinessDay3,0) - ISNULL(LEAVEDETAIL.LeaveDays3,0) ELSE 0 END
				)
				ELSE
				1.0 -  ( ISNULL(LEAVEDETAIL.LeaveDays1,0) + ISNULL(LEAVEDETAIL.LeaveDays2,0) + ISNULL(LEAVEDETAIL.LeaveDays3,0) ) 
			END
		END AS WorkDays
		...
		ELSE
			--CASE WHEN cs.ShiftCode IS NOT NULL THEN
				CASE WHEN LEAVEDETAIL.LeaveCode1 is not null 
					...
					THEN concat( LEAVEDETAIL.LeaveDays2, LEAVEDETAIL.LeaveCode2, '' | '', 1.0 - LEAVEDETAIL.LeaveDays2 , COALESCE(BUSINESSDETAIL.BusinessCode1,BUSINESSDETAIL.BusinessCode3,cs.ShiftCode ))

					when LEAVEDETAIL.LeaveCode2 is NULL AND LEAVEDETAIL.LeaveCode3 is not null
					then concat(1.0 - LEAVEDETAIL.LeaveDays3 , COALESCE(BUSINESSDETAIL.BusinessCode1,BUSINESSDETAIL.BusinessCode2,cs.ShiftCode ),  '' | '',LEAVEDETAIL.LeaveDays3, LEAVEDETAIL.LeaveCode3 )
					...
					THEN concat( BUSINESSDETAIL.BusinessDay2, BUSINESSDETAIL.BusinessCode2, '' | '', 1.0 - BUSINESSDETAIL.BusinessDay2 , COALESCE(BUSINESSDETAIL.BusinessCode3,cs.ShiftCode ))

					WHEN BUSINESSDETAIL.BusinessCode2 IS NULL AND BUSINESSDETAIL.BusinessCode3 IS NOT NULL
					THEN concat(1.0 - BUSINESSDETAIL.BusinessDay3 , COALESCE(BUSINESSDETAIL.BusinessCode2,cs.ShiftCode ),  '' | '', BUSINESSDETAIL.BusinessDay3, BUSINESSDETAIL.BusinessCode3 )
					ELSE cs.ShiftCode
				END
			--END
		END AS Symbol
		...
		,BUSINESSDETAIL.*
		,BT.*
		,CASE WHEN 1.0 - 1.0 + ISNULL(BUSINESSDETAIL.BusinessDay1,0) - ( ISNULL(LEAVEDETAIL.LeaveDays1,0) + ISNULL(LEAVEDETAIL.LeaveDays2,0) + ISNULL(LEAVEDETAIL.LeaveDays3,0)) > 0
		THEN 1.0 - 1.0 + ISNULL(BUSINESSDETAIL.BusinessDay1,0) - ( ISNULL(LEAVEDETAIL.LeaveDays1,0) + ISNULL(LEAVEDETAIL.LeaveDays2,0) + ISNULL(LEAVEDETAIL.LeaveDays3,0) ) END AS RealBusinessDay1
		, CASE WHEN 1.0 - 1.0 + ISNULL(BUSINESSDETAIL.BusinessDay2,0) - ( ISNULL(LEAVEDETAIL.LeaveDays1,0) + ISNULL(LEAVEDETAIL.LeaveDays2,0) ) > 0 
		THEN  1.0 - 1.0 + ISNULL(BUSINESSDETAIL.BusinessDay2,0) - ( ISNULL(LEAVEDETAIL.LeaveDays1,0) + ISNULL(LEAVEDETAIL.LeaveDays2,0) ) END AS RealBusinessDay2
		, CASE WHEN 1.0 - 1.0 + ISNULL(BUSINESSDETAIL.BusinessDay3,0) - ( ISNULL(LEAVEDETAIL.LeaveDays1,0) + ISNULL(LEAVEDETAIL.LeaveDays3,0) ) > 0 
		THEN 1.0 - 1.0 + ISNULL(BUSINESSDETAIL.BusinessDay3,0) - ( ISNULL(LEAVEDETAIL.LeaveDays1,0) + ISNULL(LEAVEDETAIL.LeaveDays3,0) ) END AS RealBusinessDay3
...
from    Att_Workday as aw
        ...
        cross apply (select case when cdo.DateOff is not null AND ( ld1.LeaveDayTypeCode IS NULL OR ( ld1.LeaveDayTypeCode NOT LIKE ''SIB%'' AND  ld1.LeaveDayTypeCode <> ''LS'' ) ) then ''E_FULLSHIFT-1.0-H'' end as DayOff
						,case when ld1.DurationType = ''E_FULLSHIFT'' or lde.DurationType = ''E_FULLSHIFT'' 
								...
								CASE WHEN	 
									...
									OR ( ld1.DateStart <> aw.WorkDate AND ( ld1.DurationType = ''E_FIRSTHALFSHIFT'' OR ld1.DurationType = ''E_FIRST'' ) )
									OR ( ld1.DateEND <> aw.WorkDate AND ( ld1.DurationType = ''E_LASTHALFSHIFT'' OR ld1.DurationType = ''E_LAST'' ) )
									OR ( ld1.DateEnd <> aw.WorkDate AND ( ld1.DurationType = ''E_LASTHALFSHIFT'' OR lde.DurationType = ''E_LAST''  ) )
									then concat(''E_FULLSHIFT'', ''-'', ''1.0'', ''-'', ld1.LeaveDayTypeCode)

									when lde.DurationType = ''E_FULLSHIFT'' 
									OR ( lde.DateStart <> aw.WorkDate AND ( lde.DurationType = ''E_FIRSTHALFSHIFT'' OR lde.DurationType = ''E_FIRST''  ) )
									OR ( lde.DateEnd <> aw.WorkDate AND ( lde.DurationType = ''E_LASTHALFSHIFT'' OR lde.DurationType = ''E_LAST''  ) )
									OR ( lde.DateEnd <> aw.WorkDate AND ( lde.DurationType = ''E_LASTHALFSHIFT'' OR lde.DurationType = ''E_LAST''  ) )
		...
		cross apply ( SELECT
						case when tbt1.DurationType = ''E_FULLSHIFT'' OR tbt2.DurationType = ''E_FULLSHIFT''
								...
								CASE WHEN
									...
									OR ( tbt1.DateFrom <> aw.WorkDate AND ( tbt1.DurationType = ''E_FIRSTHALFSHIFT'' OR tbt1.DurationType = ''E_FIRST''  ) )
									OR ( tbt1.DateTo <> aw.WorkDate AND ( tbt1.DurationType = ''E_LASTHALFSHIFT'' OR tbt1.DurationType = ''E_LAST''  ) )
									OR ( tbt1.DateFrom <> aw.WorkDate AND tbt1.DateTo <> aw.WorkDate AND tbt2.DurationType = ''E_FIRST_AND_LAST'' )
									then concat(''E_FULLSHIFT'', ''-'', ''1.0'', ''-'', tbt1.BusinessTravelCode)
											
									WHEN
									tbt2.DurationType = ''E_FULLSHIFT'' 
									OR ( tbt2.DateFrom <> aw.WorkDate AND ( tbt2.DurationType = ''E_FIRSTHALFSHIFT'' OR tbt2.DurationType = ''E_FIRST'' ) )
									OR ( tbt2.DateTo <> aw.WorkDate AND ( tbt2.DurationType = ''E_LASTHALFSHIFT'' OR tbt2.DurationType = ''E_LAST'' ) )
        ...
        cross apply (select left(BUSINESS.StringBusinessCode1, charindex(''-'', BUSINESS.StringBusinessCode1) - 1) as B_DurationType1
                          ...
                          , case when left(BUSINESS.StringBusinessCode1, charindex(''-'', BUSINESS.StringBusinessCode1) - 1) = ''E_FULLSHIFT'' then ''1.0''
							end as BusinessDay1
...
from    (select tw.ProfileID AS ProfileID_tss
              , ISNULL(cs.CodeStatic,tw.ShiftCode) AS ShiftCode
              , tw.WorkDays
         from   #TbWorkday as tw
		 left join Cat_Shift cs on cs.Code = tw.ShiftCode AND cs.IsDelete IS NULL
         where  1 = 1
...
(
SELECT	ProfileID
		,CASE WHEN ShiftCode = ''2'' AND WorkDays >= 0.5 AND LeaveDays3 IS NULL then 0.5 
		WHEN ShiftCode = ''3'' AND WorkDays = 1 then 7.5
		WHEN ShiftCode = ''3'' AND WorkDays = 0.5 AND LeaveDays2 IS NULL then 4.0
		WHEN ShiftCode = ''3'' AND WorkDays = 0.5 AND LeaveDays3 IS NULL then 3.5
		END AS NS1
FROM	#TbWorkday tw
WHERE	--tw.DataType = ''E_VALID''
		tw.DataType = ''E_VALID'' OR tw.DataType IS NULL
)
...
--select * from #TbWorkday where profileid =''3B1BC31F-9996-4EFD-9681-1290B4B0220E'' order by workdate
...
--and profileid =''3B1BC31F-9996-4EFD-9681-1290B4B0220E''
--and NS2 = 0.25
