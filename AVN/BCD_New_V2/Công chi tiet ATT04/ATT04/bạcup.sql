...
select ' + @Top
      ...
      , aw.WorkDate
      , cs.ShiftCode
      , case when aw.Type = ''E_NORMAL'' and cs.ShiftCode is not null then CASE WHEN 1.0 - 1.0 + isnull(LEAVEDETAIL.LeaveDays1, 0) - isnull(LEAVEDETAIL.LeaveDays2, 0) - isnull(LEAVEDETAIL.LeaveDays3, 0) > 0  
	  
	  THEN  1.0 - isnull(LEAVEDETAIL.LeaveDays1, 0) 
	  ELSE 1.0 -  isnull(LEAVEDETAIL.LeaveDays2, 0) - isnull(LEAVEDETAIL.LeaveDays3, 0) 
	  END  
        end as WorkHours
	  ...
	  , CASE WHEN 1.0 - 1.0 + isnull(LEAVEDETAIL.LeaveDays1, 0) - isnull(LEAVEDETAIL.LeaveDays2, 0) - isnull(LEAVEDETAIL.LeaveDays3, 0) > 0  
	  
	  THEN  1.0 - 1.0 + isnull(LEAVEDETAIL.LeaveDays1, 0) - isnull(LEAVEDETAIL.LeaveDays2, 0) - isnull(LEAVEDETAIL.LeaveDays3, 0)
	  ELSE 0 END AS LeaveDays1
...
select * from #TbWorkday where profileid =''E7430109-33AA-4B8F-82FD-79B7BFF3B07B'' order by workdate
...
select  NULL AS STT
		...
		, tsbl.*
		, tsld.*
		--, isnull(tsld.TotalPaidLeaveDay,0) as "111111"
		--, isnull(tsld.HLD,0) as "222222"
		--, isnull(tsbl.TotalBusinessTravelDays,0) as "3333333"
		--, isnull(tswd.TotalWorkHours,0)  as "4444444"
		--, isnull([WFH],0)  as "555"
