...
select ' + @Top+ '
		...
		,CASE WHEN cs.ShiftCode = ''2'' AND  LeaveDays1 IS NULL AND LeaveDays3 IS NULL AND ISNULL(BUSINESSDETAIL.BusinessDay1,0) + ISNULL(BUSINESSDETAIL.BusinessDay3,0) 
			...
			+ CASE WHEN NOT (cga.AttendanceMethod = ''E_TAM'' AND ( aw.FirstInTime IS NULL OR aw.LastOutTime IS NULL or aw.LateEarlyDuration > 0)) AND ISNULL(BUSINESSDETAIL.BusinessDay2,0) = 0
