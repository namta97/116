...
DECLARE @AmountCondition nvarchar(500) = ''
...
'	
	...
	select 
				...
				,csct.SalaryClassName, cptl.PositionName, cjttl.JobTitleName,ISNULL(cetl.EN,'' '') AS EmploymentType,cetl2.EN AS LaborType,cnet.NameEntityName AS EmployeeGroupName, cwptl.WorkPlaceName, hp.DateHire, hp.DateQuit,hp.DateEndProbation
				,cetl3.EN AS EmpStatus,sup.MonthYear,sup.Amount AS AdvanceAmount
				...
				,supi.Amount AS PayPaidType,sup.AccountNo,cb.GroupBank, cb.BankName,supi2.Amount AS BranchName,cb.BankName + '', '' + ISNULL(supi2.Amount,'''') AS BankBranchName
				, CASE WHEN cb.GroupBank IN(''ICB'', ''BIDV'', ''VCB'',''NN'') THEN ISNULL(cb.BankName,''NULL'') ELSE @OrtherGroupBank END AS GroupBankName
...
'	
	...
	SELECT		*
				...
				THEN ',ROW_NUMBER() OVER (ORDER BY GroupBankName,BankName,BranchName, DivisionName, CenterName,DepartmentName, SectionName ,WorkPlaceName, UnitName, EmployeeGroupName, LaborType, SalaryClassName, EmploymentType,CodeEmp) as STT' 
				ELSE ',ROW_NUMBER() OVER (ORDER BY DivisionName, CenterName,DepartmentName, SectionName ,WorkPlaceName, UnitName, EmployeeGroupName, LaborType, SalaryClassName, EmploymentType,CodeEmp ) as STT' END+'
				...
				,NULL AS "hwh.EmployeeGroupID", NULL AS "hwh.LaborType", NULL AS "AmountCondition"	
				,NULL AS "sup.EmpStatus",NULL AS "IsAdvance", NULL AS "IsNotAdvance", NULL AS "IsAdvanceBanking"
				--, NULL AS "hwh.CostCentreID", NULL AS "csc.AbilityTitleID"
	INTO		#Results
	FROM		#Sal_UnusualPay_New
	'
set @queryPageSize = ' 
	...
	FROM #Results 
	WHERE STT BETWEEN('+CAST(@PageIndex AS varchar)+' -1) * '+CAST(@PageSize AS varchar)+' + 1 AND((('+CAST(@PageIndex AS varchar)+' -1) * '+CAST(@PageSize AS varchar)+' + 1) + '+CAST(@PageSize AS varchar)+') - 1
	ORDER BY STT
...
exec(@InsertTable + @getdata +@query +@query2 + @queryPageSize )
