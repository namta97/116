...
SET @query= '
	...
	select		' +@Top+ ' 
				hwh.ProfileID , hp.CodeEmp, hp.ProfileName, hwh.OrganizationStructureID AS OrgstructureID
...
SET @query3 = '
 ...
 SET @query4 ='
	...
	SELECT		aao1.*, aao2.RealYearlyAnnualLeave, aao2.TotalUsedAnnual, aao2.RemainAnnualLeave, ROW_NUMBER() OVER ( ORDER BY E_BRANCH_E,OrderNumDI,E_DIVISION_E,OrderNumDE,E_DEPARTMENT_E ) as RowNumber
	INTO		#Results_Org
...
set @queryPageSize = ' 
	...
	DROP TABLE #tblPermission, #Hre_WorkHistory, #Infor_Hre_WorkHistory, #Att_AnnualDetail_Data, #Att_Annual_Detail_Pivot, #Results, #TbOrgUnit, #Att_Annual_Org1, #Att_Annual_Org2, #Results_Org
...
--exec( @getdata +@query + @query2 + @query3 + @queryPageSize )
