...
where   aop.IsDelete is null
		...
		and aop.WorkDateRoot between @DateStart and @DateEnd
		and not exists ( select tov.OvertimePlanID from #Att_Overtime as tov where  tov.OvertimePlanID = aop.ID )
		and exists (select tw.ProfileID from #TbWorkhistory as tw where tw.ProfileID = aop.ProfileID)
print (''#Att_OvertimePlan'');
...
SELECT		ProfileID, WorkDateRoot, SUM(ConfirmHours) AS ConfirmHours_Sum
INTO		#Att_Overtime_Sum
FROM    (
			select	tov.ProfileID, tov.WorkDateRoot,tov.ConfirmHours
			from	#Att_Overtime as tov
			union all
			select	topl.ProfileID, topl.WorkDateRoot, topl.ApproveHours
			from	#Att_OvertimePlan as topl
        ) as DataOT
GROUP BY	ProfileID, WorkDateRoot
...
FROM	#TbWorkday tw
WHERE	tw.ShiftID IS NOT NULL AND tw.DataType = ''E_VALID'' OR tw.DataType IS NULL
GROUP BY ProfileID
...
Drop table #SumNightShiftHours, #Att_Overtime, #Att_OvertimePlan
