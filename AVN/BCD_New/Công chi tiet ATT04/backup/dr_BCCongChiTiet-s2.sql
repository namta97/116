...
;WITH WorkHistory AS
(
select		'+@Top+' hwh.ProfileID , hp.CodeEmp, hp.ProfileName, cou.E_BRANCH AS DivisionName ,cou.E_UNIT AS CenterName,cou.E_DIVISION AS DepartmentName,cou.E_DEPARTMENT AS SectionName,cou.E_TEAM AS UnitName
...
select ' + @Top+ '
		...
		,aw.WorkDate
		,cs.ShiftCode
		,CASE WHEN cs.ShiftCode = ''2'' AND CASE WHEN cga.AttendanceMethod = ''E_TAM'' AND ( aw.FirstInTime IS NULL OR aw.LastOutTime IS NULL or aw.LateEarlyDuration > 0) 
			THEN ISNULL(BUSINESSDETAIL.BusinessDay1,0) + ISNULL(BUSINESSDETAIL.BusinessDay3,0) ELSE 1 END 
			- (ISNULL(LEAVEDETAIL.LeaveDays1,0) + ISNULL(LEAVEDETAIL.LeaveDays2,0) + ISNULL(LEAVEDETAIL.LeaveDays3,0)) >= 0.5 AND LeaveDays3 IS NULL then 0.5 

		WHEN cs.ShiftCode = ''3'' AND CASE WHEN cga.AttendanceMethod = ''E_TAM'' AND ( aw.FirstInTime IS NULL OR aw.LastOutTime IS NULL OR aw.LateEarlyDuration > 0) 
			THEN ISNULL(BUSINESSDETAIL.BusinessDay1,0) + ISNULL(BUSINESSDETAIL.BusinessDay2,0) + ISNULL(BUSINESSDETAIL.BusinessDay3,0) ELSE 1 END 
			- (ISNULL(LEAVEDETAIL.LeaveDays1,0) + ISNULL(LEAVEDETAIL.LeaveDays2,0) + ISNULL(LEAVEDETAIL.LeaveDays3,0)) = 1 then 7.5

		WHEN cs.ShiftCode = ''3'' AND CASE WHEN cga.AttendanceMethod = ''E_TAM'' AND ( aw.FirstInTime IS NULL OR aw.LastOutTime IS NULL OR aw.LateEarlyDuration > 0) 
			THEN ISNULL(BUSINESSDETAIL.BusinessDay1,0) + ISNULL(BUSINESSDETAIL.BusinessDay2,0) + ISNULL(BUSINESSDETAIL.BusinessDay3,0) ELSE 1 END 
			- (ISNULL(LEAVEDETAIL.LeaveDays1,0) + ISNULL(LEAVEDETAIL.LeaveDays2,0) + ISNULL(LEAVEDETAIL.LeaveDays3,0)) = 0.5 AND LeaveDays2 IS NULL then 4.0

		WHEN cs.ShiftCode = ''3'' AND CASE WHEN cga.AttendanceMethod = ''E_TAM'' AND ( aw.FirstInTime IS NULL OR aw.LastOutTime IS NULL OR aw.LateEarlyDuration > 0)
			THEN ISNULL(BUSINESSDETAIL.BusinessDay1,0) + ISNULL(BUSINESSDETAIL.BusinessDay2,0) + ISNULL(BUSINESSDETAIL.BusinessDay3,0) ELSE 1 END 
			- (ISNULL(LEAVEDETAIL.LeaveDays1,0) + ISNULL(LEAVEDETAIL.LeaveDays2,0) + ISNULL(LEAVEDETAIL.LeaveDays3,0)) = 0.5 AND LeaveDays3 IS NULL then 3.5
		END AS NS1


		,CASE WHEN cs.ShiftCode = ''2'' AND LeaveDays3 IS NULL THEN (BUSINESSDETAIL.BusinessDay1,0) + ISNULL(BUSINESSDETAIL.BusinessDay3,0) 
			+ CASE WHEN cga.AttendanceMethod = ''E_TAM'' AND ( aw.FirstInTime IS NULL OR aw.LastOutTime IS NULL or aw.LateEarlyDuration > 0) AND ISNULL(BUSINESSDETAIL.BusinessDay3,0) > 0
			THEN 0 ELSE 0.5 END
			- (ISNULL(LEAVEDETAIL.LeaveDays1,0) + ISNULL(LEAVEDETAIL.LeaveDays3,0)) >= 0.5 THEN 0.5

			WHEN cs.ShiftCode = ''3'' AND LeaveDays1 IS NULL AND LeaveDays2 IS NULL AND LeaveDays3 IS NULL AND cga.AttendanceMethod = ''E_TAM'' AND ( aw.FirstInTime IS NULL OR aw.LastOutTime IS NULL or aw.LateEarlyDuration > 0)
			THEN 7.5

			WHEN cs.ShiftCode = ''3'' AND LeaveDays2 IS NULL AND LeaveDays1 IS NULL THEN (BUSINESSDETAIL.BusinessDay1,0) + ISNULL(BUSINESSDETAIL.BusinessDay3,0) 
			+ CASE WHEN cga.AttendanceMethod = ''E_TAM'' AND ( aw.FirstInTime IS NULL OR aw.LastOutTime IS NULL or aw.LateEarlyDuration > 0) AND ISNULL(BUSINESSDETAIL.BusinessDay3,0) > 0
			THEN 0 ELSE 0.5 END
			- (ISNULL(LEAVEDETAIL.LeaveDays1,0) + ISNULL(LEAVEDETAIL.LeaveDays3,0)) >= 0.5 THEN 0.5
			...
			CASE WHEN cs.ShiftCode IS NOT NULL THEN
				CASE WHEN LEAVEDETAIL.LeaveCode1 is not null 
					...
					THEN concat( LEAVEDETAIL.LeaveDays2, LEAVEDETAIL.LeaveCode2, '' | '', 
						CASE WHEN BUSINESSDETAIL.BusinessCode1 IS NULL AND BUSINESSDETAIL.BusinessCode3 IS NULL AND cga.AttendanceMethod = ''E_TAM'' AND ( aw.FirstInTime IS NULL OR aw.LastOutTime IS NULL OR aw.LateEarlyDuration > 0) THEN 0
						ELSE 1 - LEAVEDETAIL.LeaveDays2 END , COALESCE(BUSINESSDETAIL.BusinessCode1,BUSINESSDETAIL.BusinessCode3,''Ca'' + cs.ShiftCode ))

					when LEAVEDETAIL.LeaveCode2 is NULL AND LEAVEDETAIL.LeaveCode3 is not null
					then concat(
						CASE WHEN BUSINESSDETAIL.BusinessCode1 IS NULL AND BUSINESSDETAIL.BusinessCode2 IS NULL AND cga.AttendanceMethod = ''E_TAM'' AND ( aw.FirstInTime IS NULL OR aw.LastOutTime IS NULL OR aw.LateEarlyDuration > 0) THEN 0
						ELSE 1 - LEAVEDETAIL.LeaveDays3 END , COALESCE(BUSINESSDETAIL.BusinessCode1,BUSINESSDETAIL.BusinessCode2,''Ca'' + cs.ShiftCode ),  '' | '',LEAVEDETAIL.LeaveDays3, LEAVEDETAIL.LeaveCode3 )
					...
					THEN concat( BUSINESSDETAIL.BusinessDay2, BUSINESSDETAIL.BusinessCode2, '' | '', 
						CASE WHEN BUSINESSDETAIL.BusinessCode3 IS NULL AND cga.AttendanceMethod = ''E_TAM'' AND ( aw.FirstInTime IS NULL OR aw.LastOutTime IS NULL OR aw.LateEarlyDuration > 0) THEN 0
						ELSE 1 - BUSINESSDETAIL.BusinessDay2 END , COALESCE(BUSINESSDETAIL.BusinessCode3,''Ca''+ cs.ShiftCode ))

					WHEN BUSINESSDETAIL.BusinessCode2 IS NULL AND BUSINESSDETAIL.BusinessCode3 IS NOT NULL
					THEN concat(
						CASE WHEN BUSINESSDETAIL.BusinessCode2 IS NULL AND cga.AttendanceMethod = ''E_TAM'' AND ( aw.FirstInTime IS NULL OR aw.LastOutTime IS NULL OR aw.LateEarlyDuration > 0) THEN 0
						ELSE 1 - BUSINESSDETAIL.BusinessDay3 END , COALESCE(BUSINESSDETAIL.BusinessCode2,''Ca'' + cs.ShiftCode ),  '' | '', BUSINESSDETAIL.BusinessDay3, BUSINESSDETAIL.BusinessCode3 )
					WHEN cga.AttendanceMethod = ''E_TAM'' AND ( aw.FirstInTime IS NULL OR aw.LastOutTime IS NULL OR aw.LateEarlyDuration > 0) THEN  ''0.0'' + ''Ca'' + cs.ShiftCode 
...
select		' + @Top + '
			...
			,CASE WHEN tw.DataType = ''E_INVALID'' THEN ''???'' 
				...
				WHEN tw.LeaveCode1 =''H'' THEN ISNULL(CONVERT(VARCHAR(10),aos.ConfirmHours_Sum), ''H'') 
...
select  NULL AS STT
		...
		, ISNULL(E,0) AS E
		, ISNULL(G,0) + ISNULL(G1,0) + ISNULL(G2,0) + ISNULL(G3,0) + ISNULL(G4,0) + ISNULL(G5,0) + ISNULL(G6,0) + ISNULL(G7,0) + ISNULL(G8,0) + ISNULL(G9,0) + ISNULL(G10,0) + ISNULL(G11,0) 
		+ ISNULL(G12,0) + ISNULL(G13,0) + ISNULL(G14,0) + ISNULL(G15,0) + ISNULL(G16,0) + ISNULL(G17,0)  AS G
		...
		,GETDATE() AS DateExport
	  	,NULL AS "hp.CodeEmp",NULL AS "cos.OrderNumber",NULL AS "hwh.EmploymentType",NULL AS "hwh.SalaryClassID" ,NULL AS "hwh.PositionID", NULL AS "hwh.JobTitleID", NULL AS "hp.DateHire"
		,NULL AS "hp.DateEndProbation", NULL AS "hp.DateQuit",NULL AS "hwh.WorkPlaceID"
