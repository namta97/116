ALTER proc dr_OTComparistion2Year
...
declare @GroupA varchar(max)
SELECT @GroupA = COALESCE(@GroupA+'','', '''')  + Code FROM Cat_SalaryClass where IsDelete is NULL AND Code NOT IN (select Result from GetSplitString(@GroupB)) AND Code NOT IN (''E-STF'',''.'') order by Code ASC
...
SELECT	OrgstructureID,E_BRANCH_CODE, E_DIVISION_CODE, E_DEPARTMENT_CODE
INTO	#UnitCode 
FROM	dbo.Cat_OrgUnit WHERE IsDelete IS NULL 

SELECT	a.ID AS OrgstructureID ,a.Code, b.OrgStructureName, ROW_NUMBER() OVER(PARTITION BY a.Code ORDER BY a.DateUpdate DESC) AS rk	 
INTO	#NamEnglish1 
...
SELECT		a.OrgstructureID,E_BRANCH_CODE, E_DIVISION_CODE, E_DEPARTMENT_CODE, b.OrgStructureName AS E_BRANCH_E, d.OrgStructureName AS E_DIVISION_E, e.OrgStructureName AS E_DEPARTMENT_E
INTO		#NameEnglishORG
FROM		#UnitCode a
...
DELETE #TbOrgUnit WHERE OrgName IS NULL OR E_BRANCH_E IS NULL
...
into    #TbDataOT
FROM    (
			select	tov.ID, tov.ProfileID, tov.MonthYear, tov.ConfirmHours
			from	#TbOvertime as tov
			union all
			select	topl.ID, topl.ProfileID, topl.MonthYear, topl.ApproveHours
...
outer apply 
		(select *
         ...
         where  tw.ProfileID = DataOT.ProfileID
                and tw.MonthYear = (SELECT  max(t.MonthYear)
         ...
         from   #TbOrgUnit as tou
                ...
                cross apply (select count(distinct tw.ProfileID) as HeadCount
                             ...
                             where  tw.E_BRANCH_CODE = tou.E_BRANCH_CODE
                                    ...
                                    AND NOT ( tw.Status = ''InActive'' AND tw.MonthYear <> yo.MonthYear )
                ...
                cross apply (select count(distinct tw.ProfileID) as HeadCount
                             ...
                             where  ( tw.E_DIVISION_CODE = tou.E_DIVISION_CODE
									OR tw.E_DIVISION_CODE +''DIVIMANAGER'' = tou.E_DIVISION_CODE )
                                    and tw.DateEffective <= yo.DateEnd
                                    and tw.GroupType = ''GroupA''
                                    AND NOT ( tw.Status = ''InActive'' AND tw.MonthYear <> yo.MonthYear )
                                    and tw.DateEffective = (select  max(t.DateEffective)
                ...
                cross apply (select count(distinct tw.ProfileID) as HeadCount
                             ...
                             where  ( tw.E_DEPARTMENT_CODE = tou.E_DEPARTMENT_CODE
									OR tw.E_DEPARTMENT_CODE +''DEPMANAGER'' = tou.E_DEPARTMENT_CODE )
                                    and tw.DateEffective <= yo.DateEnd
                                    and tw.GroupType = ''GroupA''
									AND NOT ( tw.Status = ''InActive'' AND tw.MonthYear <> yo.MonthYear )
                                    and tw.DateEffective = (select  max(t.DateEffective)
         ...
         from   #TbOrgUnit as tou
                ...
                cross apply (select count(distinct tdo.ProfileID) as HeadCountOT
                             ...
                             where  ( tdo.E_DIVISION_CODE = tou.E_DIVISION_CODE
									OR tdo.E_DIVISION_CODE +''DIVIMANAGER'' = tou.E_DIVISION_CODE )
                                    and tdo.MonthYear = yo.MonthYear
                                    and tdo.GroupType = ''GroupA''
                            ) as Division
                cross apply (select count(distinct tdo.ProfileID) as HeadCountOT
                             ...
                             where  ( tdo.E_DEPARTMENT_CODE = tou.E_DEPARTMENT_CODE
									OR tdo.E_DEPARTMENT_CODE +''DEPMANAGER'' = tou.E_DEPARTMENT_CODE )
                                    and tdo.MonthYear = yo.MonthYear
                                    and tdo.GroupType = ''GroupA''
                            ) as Department
...
from    (select tou.ID
        ...
        ) as DataOT pivot ( max(AvgOTHours) for MonthYear in (' + @ListCol + ') ) as pv;
...
drop table #TbPermission, #TbOrgUnit, #TbWorkhistory, #TbOvertime, #TbOvertimePlan, #TbDataOT, #TbHCGroupA, #TbHCOTGroupA, #TbOTGroupA, #TbAvgOTHoursGroupA, #TbHCGroupB, #TbHCOTGroupB, #TbOTGroupB, #TbAvgOTHoursGroupB
			,#UnitCode,#NamEnglish1,#NamEnglish,#NameEnglishORG;
';
