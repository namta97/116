...
BEGIN
	...
	SET @index = charindex('(condi.DL ','('+@tempID,0) 
	if(@index > 0)
	BEGIN
		set @TempCondition = 'and ('+@tempID
		set @condition = REPLACE(@condition,@TempCondition,'')
		set @TotalDL = REPLACE(@TempCondition,'(condi.DL in ','')
...
SELECT		ProfileID, ISNULL(COUNT(*),0) AS CountDependant
INTO		#CountDependant
FROM		Hre_Dependant 
...
;WITH Results AS
(
select	twh.*, CAST(dbo.VnrDecrypt(sbs.E_GrossAmount) AS float) AS GrossAmount, CAST(dbo.VnrDecrypt(sbs.E_AdvanceSalary) as float) AS AdvanceAmount ,CAST(dbo.VnrDecrypt(sbs.E_InsuranceAmount) as float) AS InsuranceAmount
		'+@AlowanceAlias+'
		...
		,NULL AS "condi.WorkPlaceID",NULL AS "condi.EmployeeGroupID", NULL AS "condi.LaborType"
		,NULL AS "condi.EmpStatus",NULL AS "CheckData", NULL AS ''ag.GradeAttendanceID'', NULL AS "condi.DL"
		,NULL AS "IsAdvance"
		,Agr.ProfileID AS AgrProfileID
		,ROW_NUMBER() OVER ( ORDER BY DivisionName, CenterName,DepartmentName, SectionName ,WorkPlaceName, UnitName, EmployeeGroupName, twh.LaborType, SalaryClassName, twh.EmploymentType,CodeEmp ) as RowNumber
from    #TbWorkHistory as twh
		...
		LEFT JOIN #Sal_BasicSalary sbs on sbs.ProfileID = twh.ProfileID
...
INTO	#Results
from	Results
where	1= 1 '+@TotalPaidDays+'  '+@CheckData+' '+@TotalDL+'
...
drop table #tblPermission, #TbSumBusinessLeave,#Sal_BasicSalary, #TbSumLeaveDay, #TbSumWorkDay, #TbWorkday, #TbWorkHistory, #Results;
