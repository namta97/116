...
-- Tach dieu kien
	...
	BEGIN
		...
		BEGIN
			set @tempCodition = 'and ('+@tempID
			set @condition = REPLACE(@condition,@tempCodition,'')
			set @TotalPaidDays = REPLACE(@tempCodition,'(TotalPaidDays in ','')
		...
		BEGIN
			set @tempCodition = 'and ('+@tempID
			set @condition = REPLACE(@condition,@tempCodition,'')
			set @TotalDL = REPLACE(@tempCodition,'(DL in ','')
...
select ' + @Top+ '
		...
		,aw.WorkDate
		,cs.ShiftCode
		,CASE WHEN cs.ShiftCode = ''2'' AND CASE WHEN cga.AttendanceMethod = ''E_TAM'' AND ( aw.FirstInTime IS NULL OR aw.LastOutTime IS NULL ) 
		...
		WHEN cs.ShiftCode = ''3'' AND CASE WHEN cga.AttendanceMethod = ''E_TAM'' AND ( aw.FirstInTime IS NULL OR aw.LastOutTime IS NULL ) 
			THEN ISNULL(BUSINESSDETAIL.BusinessDay1,0) + ISNULL(BUSINESSDETAIL.BusinessDay2,0) + ISNULL(BUSINESSDETAIL.BusinessDay3,0) ELSE 1 END 
		...
		END AS BusinessDay
		,CASE WHEN cs.ShiftCode is not null THEN
			CASE WHEN cga.AttendanceMethod = ''E_TAM'' AND ( aw.FirstInTime IS NULL OR aw.LastOutTime IS NULL ) THEN 0
...
select	twh.*,alp.*
		...
		,ISNULL(tsld.HLD,0) AS HLD, ISNULL(tsld.AN,0) AS AN, ISNULL(tsld.TotalPaidLeaveDay,0) - ISNULL(tsld.AN,0) AS TotalPaidLeaveDay, ISNULL(tsld.DL,0) AS DL, ISNULL(tsld.TotalNoPaidLeaveDay,0) AS TotalNoPaidLeaveDay
...
drop table #tblPermission, #TbSumBusinessLeave, #TbSumLeaveDay, #TbSumWorkDay, #TbWorkday, #TbWorkHistory, #Results;
