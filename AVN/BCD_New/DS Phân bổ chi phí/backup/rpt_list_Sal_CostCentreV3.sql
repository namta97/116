...
'			
	...
	select		' +@Top +'
				...
				,cou.E_BRANCH AS DivisionName, cou.E_UNIT AS CenterName, cou.E_DIVISION AS DepartmentName, cou.E_DEPARTMENT AS SectionName, cou.E_TEAM AS UnitName,Cost.Code AS CostCentreName, Cost.ID as CostCentreID
	FROM		Hre_Profile hp
	...
	Select		lec.ProfileID,lec.WorkHistoryID, lec.DivisionName, lec.CenterName,lec.DepartmentName, lec.SectionName,lec.UnitName, lec.CostCentreName
				,cou.E_BRANCH AS DivisionName_Share , cou.E_UNIT AS CenterName_Share ,cou.E_DIVISION AS DepartmentName_Share , cou.E_DEPARTMENT AS SectionName_Share ,cou.E_TEAM AS UnitName_Share, lec.CostCentreName AS CostCentreName_Share
...
'	
	...
	UNION
				...
				SELECT		' +@Top + '
							...
							,ROW_NUMBER() OVER(PARTITION BY sccs.ProfileID ORDER BY sccs.Rate DESC) AS LevelRoot,CASE WHEN cost.Code = lec.CostCentreName THEN 1 ELSE 0 END AS IsDuplicate, NULL AS STT
	...
	SELECT		lecs.STT,lecs.ProfileID,hp.CodeEmp, hp.ProfileName
				...
				,lecs.RateShare, lecs.DateEffectFrom, lecs.DateEffectTo,lecs.LevelRoot, 1 AS IsDisPlay, IsDuplicate , 0 AS IsErrorData
